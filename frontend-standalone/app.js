// URL de l'API (configurée dans index.html)
...
function renderTracks(tracks, speakers, segments) {
    ...
    tracks.forEach((track, index) => {
        ...
        const audioUrl = `${API_URL}/api/download/${currentJobId}/${track.filename}`;
...
async function downloadTrack(filename, speaker) {
    ...
    try {
        const response = await fetch(`${API_URL}/api/download/${currentJobId}/${filename}?name=${encodeURIComponent(downloadName)}`);
...
async function downloadAll() {
    ...
    try {
        const response = await fetch(`${API_URL}/api/download-all/${currentJobId}`, {
            method: 'POST',
            headers: {
...
function reset() {
    ...
    if (currentJobId) {
        fetch(`${API_URL}/api/cleanup/${currentJobId}`, { method: 'DELETE' }).catch(() => {});
        currentJobId = null;
    }
...
window.addEventListener('beforeunload', () => {
    ...
    if (currentJobId) {
        navigator.sendBeacon(`${API_URL}/api/cleanup/${currentJobId}`);
    }
});
